...
export const MicButton = forwardRef<MicButtonHandle, MicButtonProps>(function MicButton(
  ...
  const applyVadConfig = useCallback(
    (config: VadConfig) => {
  ...
  useImperativeHandle(
    ...
    () => ({
      ...
      stop: () => {
        ...
        if (mountedRef.current) onStateChangeRef.current("idle");
      },
      setVadConfig: applyVadConfig,
    }),
    [disabled, locked, onLockedTap, startVAD, applyVadConfig],
  );
  ...
  }, [disabled, locked, onLockedTap, state, startVAD]);
