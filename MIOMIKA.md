...
|------|---------|-------|---------|--------|------|
| 2026-06-05 | Cursor Composer — live voice regression hotfix | Voice | **REVERT** round-3 gapless `pcm-playback-processor.js` AudioWorklet (noise-only output, no speech). Restored round-2 `media-handler.ts`: per-chunk `AudioBufferSourceNode` @ 24 kHz + 64-sample fades + 40ms schedule-ahead (faint click OK). `/talk`: mount effect auto-starts live kickoff with voice on SPA entry — `unlockTtsPlayback()` + `ensurePlaybackUnlocked()`; mic orb stays separate (`awaitingMic`). Deleted `public/pcm-playback-processor.js`. `pcm-processor.js` unchanged. tsc PASS. Pushed → main. | Round-3 click fix deferred. | Mike: Home → /talk — greeting speaks on entry; replies are clear voice not noise. |
