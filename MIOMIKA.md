...
| 2026-08-07 | Cursor · Admin V3 command center | Admin | **V3:** audit smart search (`7fb545c`); attention engine + overview CC (`a7da553`); watchboard + reminders SQL/API (`6326935`); live pulse (`commit 4`). Real data only. Run `supabase-admin-reminders.sql` once. | Reminders UI muted until SQL applied. | Mike: apply reminders SQL · /admin Overview at 1280×800 · audit name search · pulse feed. |
| 2026-08-07 | Cursor · Admin mobile scroll + responsive | Admin | **Scroll:** content pane `flex-1 min-h-0 overflow-y-auto` inside `h-[100dvh]` (globals lock html/body). **Responsive:** AdminHeader 2-row mobile, range/tab h-scroll, KPI 1/2/4 cols, chart 200px + preserveStartEnd, Users card layout &lt;md, table overflow wrappers, stacked filters/actions. Commits `f672037` + responsive. | — | Mike: /admin at 390px + desktop · confirm scroll + no page-level sideways scroll. |
| 2026-08-07 | Cursor · Admin crash + internal filter | Admin | **Root cause:** `components/admin/ui.tsx` had `"use client"` while server layout/pages called `tint()` → `Attempted to call tint() from the server but tint is on the client` (digest on /admin). Fix `2ef472f` removes `"use client"`. Commit 2: `INTERNAL_EMAILS` filter via `lib/admin/internal.ts` · overview growth/revenue KPIs exclude · Revenue MRR excludes + Internal (N) · Users/Cost chip. | — | Mike: /admin after deploy · set INTERNAL_EMAILS if needed. |
| 2026-08-07 | Cursor · Admin V2 Beauty Pass | Admin | **Visual-only beauty.** Commit 1 `cd0a6ee` design system (`components/admin/ui.tsx` palette + KpiCard/StatusPill/TierBadge/Avatar + chart theme + header/nav/range). Commit 2 applies vivid semantic colors across Overview/Users/Revenue/Cost/Health/Audit. Quicksand numbers · ฿ · · not —. Zero logic/API changes. tsc + eslint 0. | — | Mike: /admin visual smoke across tabs + ranges. |
| 2026-08-07 | Cursor · Admin Console V2 | Admin | **Time ranges + graphs + Health + users power.** Commits: `4e49253` range framework/shell; `5c1a810` overview KPIs+recharts; `2ec1818` Health tab; `a7ef9e9` users filters/CSV; cockpit adjust_* + per-user cost chart. Signups from auth.users.created_at only. Pack revenue from room_credit_ledger (credit_ledger/webhook_events hold no payment amounts). | Subscription charge amounts not in DB · no time-series sub revenue. | Mike: /admin smoke · range picker across tabs · Health · user CSV · adjust confirm. |
| 2026-08-07 | Cursor · Google Startup eligibility | Marketing | **Public landing eligibility** · Commit 1 skipped (`/` already public in `app/page.tsx`). **(2)** `2c31a23` Inside Miomika 3-up press screenshots. **(3)** `dd6234c` founder LinkedIn `rel=me` + `PublicSiteFooter` on `/`, `/help`, `/legal/*`. **(4)** `0b1f2b9` Start free / Sign in / live-now CTAs. tsc + eslint 0 errors. | Legal Terms/Privacy still say beta (left untouched per brief). | Mike: incognito miomika.com · confirm screenshots, founder LinkedIn, footer; re-apply Google for Startups. |
| 2026-07-23 | Cursor — stuck-in-Thai + first-word silence | Talk / Voice | **(1)** Adjust reply-language pill fires `onLanguageChange` → live `setSessionLanguage` (profile alone left mid-session stuck). **(2)** TTS `ensurePlaybackRunning()` awaits AudioContext `running` before first `el.play()`. Base `7f49c19`. `npm run predeploy` PASS. | — | Mike: mid-Talk language switch + first utterance audible on cold session. |
| 2026-07-22 | Cursor — SW BUILD_ID root cure | Infra / PWA | **Root cause:** Vercel had no `buildCommand` → plain `next build` skipped `generate-build-version.mjs` → committed `sw.js` frozen at July-16 BUILD_ID. **(1)** `vercel.json` `buildCommand` runs generator every deploy. **(2)** Cache-scope fix moved into generator TEMPLATE (source of truth). **(3)** Regenerated `sw.js` + `version.json` to HEAD `04f0634c3892`. `npm run predeploy` PASS. | — | Mike: next Vercel deploy → fresh SW auto-updates stuck July-16 clients; no "please refresh". |
| 2026-07-21 | Cursor — natural bilingual + SW build cache | Brain / PWA | **(1)** Persona: keep ค่ะ/นะคะ in both langs; forbid Thai self-word/nouns mid-English only. **(2)** SW `networkFirst`/`cacheFirst` lookups scoped to current build cache (no cross-build stale shell). `npm run predeploy` PASS. | — | Mike: English Talk — no "หนู just woke up"; post-deploy visit — no "please refresh". |
| 2026-07-20 | Cursor — error boundary auto-recover | Infra / PWA | **global-error + (app)/error** silently reload once on transient first-load/chunk errors (stale SW shell after deploy); `sessionStorage` guard prevents loops; `PwaUpdateManager` clears guards on successful mount. `npm run predeploy` PASS. | — | Mike: hard-refresh after deploy — no "please refresh the page" flash. |
| 2026-07-20 | Cursor — inline Say-It + STT lang | Bond / Vocab | **Removed practice modal** — "Try saying this" records inline on compact card; STT uses headword language (matches `primaryWord`/`audioLang`), not UI lang. Mastery + progress refresh kept. `npm run predeploy` PASS. | — | Mike: Growth reveal → Try saying this → Thai STT on Thai words; Due updates. |
...
| 2026-07-20 | Cursor — calendar BKK + dead toggle | Bond / Talk | **(B)** Activity-calendar `toDateKey` / `computeStreak` use Bangkok day boundaries (UTC+7) so evening sessions light the right circle. **(C)** Removed inert Adjust "Favorite topics" toggle (no topic data source). `memory.topics` type kept. `npm run predeploy` PASS. | — | Mike: evening Bangkok session → Growth calendar lights today; Adjust Memory shows only progress + personal. |
| 2026-07-19 | Cursor — Say-It advances mastery | Vocab / Bond | **Correct SayItCheck** on word cards POSTs `/api/vocab/practiced` → `advance_word_mastery` (same spiral as conversation reuse). Phrases unaffected (no mastery row). `npm run predeploy` PASS. | — | Mike: pass Say-It on a word card → Growth mastery/level moves. |
| 2026-07-19 | Cursor — TTS orphaned punct | Voice | **`stripForTts`** collapses orphaned punctuation after parenthetical asides (`" ."` → attached/dropped) so Chirp3 no longer voices a trailing "dot". `npm run predeploy` PASS. | — | Mike: Thai reply with romanization aside — no spoken trailing click/dot. |
| 2026-07-19 | Cursor — Say-It mic + streak | Learn / Bond | **SayGame** is thin wrapper — `SayItCheck` sole mic owner (fixes dual-recorder "couldn't hear you"). **Day streak** reads `profiles.streak` (Bangkok-day) instead of UTC `computeStreak`. `npm run predeploy` PASS. | — | Mike: lesson Say-It hears + judges; Growth day streak matches daily use after evening Bangkok sessions. |
| 2026-07-19 | Cursor — Learn Say-It real verdict | Learn | **Lesson SayGame** review shows real `SayItCheck` STT verdict (same as word cards & Talk) — no more fake ear-to-ear self-approval. Lesson `onDone` still fires on first recording. `npm run predeploy` PASS. | — | Mike: lesson Say-It → record → see honest heard/match verdict, not Miomi/Me compare buttons. |
| 2026-07-19 | Cursor — language switch + name law | Talk / Brain | **Adjust language switch is real** — `setSessionLanguage` keeps live client medium in sync. **Firm CONVERSATION LANGUAGE directive** + stricter THEIR NAME (≤1/reply). `npm run predeploy` PASS. | — | Mike: Thai↔English both ways mid-session; name used at most once per reply. |
| 2026-07-19 | Cursor — adjust clearance + persona laws | Talk / Brain | **Adjust button** clears Upgrade badge (`mr-10 sm:mr-[150px]`). **Persona laws** — no third-person self-naming; requests get real delivery before any question. `npm run predeploy` PASS. | — | Mike: Adjust reachable on mobile+desktop; ask "make me feel better" → comfort first, not only a question. |
| 2026-07-19 | Cursor — streak real + adjust honesty | Bond / Phase 4 | **Streak becomes real** — `touchLastSeen` counts Bangkok-calendar days into `profiles.streak`. **Adjust menu honesty** — tone/depth/practice/memory ride every `/api/miomi` call; memory toggles are server-side gates in `buildMemoryContext`. `npm run predeploy` PASS. | — | Mike: talk two days → streak grows; Adjust → focused/playful + depth + memory off → prompt/behavior matches. |
| 2026-07-18 | Cursor — fat-output guard + hamburger fix | Brain / Talk | **`trimReply`** mechanical 3-sentence cap (question promoted, `[[CARD:` protected); hamburger `display:flex` removed so `xl:hidden` wins. `npm run predeploy` PASS. | — | Mike: fat reply → ≤3 sentences + question + card intact; desktop /talk → no hamburger. |
| 2026-07-18 | Cursor — multi-thread chat phase 3.1 | Talk | **Polish** — chats nested under Talk in rail; history filters `[kickoff]%`; drawer portals to `document.body` z-9999; tier badge crown-only on mobile. `npm run predeploy` PASS. | — | Mike: rail chats under Talk; reopen thread → no kickoff bubble; mobile drawer above badge; crown compact. |
| 2026-07-18 | Cursor — multi-thread chat phase 3 | Talk | **Threads sidebar + mobile drawer** — `ThreadsPanel` (list/rename/delete); rail hosts it; `/talk` hamburger drawer + `?thread=` / `?new=1` URL bus; Clear dispatches `miomika:threads`. `npm run predeploy` PASS. | — | Mike: expand rail → chats list; mobile menu → drawer; open/rename/delete; New chat + Clear refresh list. |
| 2026-07-18 | Cursor — multi-thread chat phase 2.1 | Talk | **Resume hotfix** — sync `kickoffSentRef` before fetches (kills greeting race); history returns language/session/exchange; restore groups user→miomi, single-lang MiniCat rows; daily-limit status → real underline button. `npm run predeploy` PASS. | Rail "Recent with Miomi" in `components/layout/Rail.tsx` is decorative dead (always "No chats yet") — Phase 3 owns that surface. | Phase 3: thread sidebar wired to real threads. |
| 2026-07-18 | Cursor — multi-thread chat phase 2 | Talk | **Client thread binding** — `MiomiTurnClient.setThreadId` on every `/api/miomi` call; member bootstrap resumes latest thread (history, no fresh hello) or creates first; Clear = New chat (POST thread + kickoff). Guests unchanged. `npm run predeploy` PASS. | — | Phase 3: thread sidebar (list / rename / switch). |
| 2026-07-18 | Cursor — multi-thread chat phase 1 | Talk | **Server-only thread model** — `0026_talk_threads.sql` (already applied in Supabase); `saveExchange`/`touchThread` bind `conversations.thread_id`; `/api/talk/threads` list+create, `/api/talk/threads/[id]` rename+soft-delete, `/api/talk/history?thread=`; `/api/miomi` optional `threadId` (legacy clients unchanged). `npm run predeploy` PASS. Zero UI. | — | Phase 2: /talk thread sidebar + client `threadId` wiring. |
| 2026-07-17 | Cursor — soft daily limit + calm voice | Talk / Voice | **Pushed `6f581b9` + `17aa467`** — (1) `/talk` daily limit → soft tappable status line (no auto-opening paywall); (2) `prompt.ts` CALM VOICE (≤1 `!`, rare interjections) + FINAL CHECK brevity. `npm run predeploy` PASS. | — | Mike: hit free daily chat cap → teal tappable line opens paywall on tap; listen for calmer TTS punctuation. |
...
| 2026-06-07 | Cursor Composer — language architecture redesign | Voice / 3B | **Language architecture redesign** — decoupled three fused concerns: (1) UI chrome = stable `profile.ui_language` / browser seed at connect only (`resolveLiveSessionLanguages`); (2) learning target = stable `profile.learning_target_language` at connect (`cardDirectionForTarget`); (3) conversational language = model-owned via strengthened connect-time `LANGUAGE CONTRACT` in `live-config.ts`. Deleted `maybeAdaptSessionLanguages` + `sendLanguageContext` + `[language_update]` mid-session nudges. Transcript routing by spoken script (`transcript-routing.ts`); `MiniCatRow` shows actual Miomi text. `resolveUiLanguage`/`resolveTargetLanguage` no longer keyword-flip per utterance. turn-controller + `modelTurnActive` untouched. check:talk 283 + tsc + lint PASS. | — | Mike: EN guest says "teach me Thai" / mixed input → UI stays EN, cards th_to_en, no mid-session flip; Miomi bubble shows what she actually said. |
| 2026-06-07 | Cursor Composer — model-turn mic gate | Voice / 3B | **Model-turn mic gate + deferred nudges** — root cause: per-chunk `playbackActive` flicker reopened mic between PCM chunks → backchannel → false `interrupted` → mid-sentence cutout. `modelTurnActive` in `media-handler.ts` gates mic from first model chunk until `turn_complete` + `waitForTurnAudioThenIdle` drain; `deferUntilPlaybackIdle` queues `sendHiddenContext` (phase nudges + content/explicit lesson intents). Gapless worklet skipped (reverted 32f9648 noise-only). turn-controller + interrupted handler untouched. check:talk 279 + tsc + lint PASS. Pushed main @ `3528821`; Vercel Ready — `buildId=3528821b8d46` (built 2026-06-07T12:16:53Z). Live bundle `03~6p37j7enva.js`: `modelTurnActive` + `deferUntilPlaybackIdle` + `signalModelTurnComplete` + `endModelTurnWhenDrained`. | — | Mike: /talk debug overlay — no spurious user transcript / interrupted while `speaking`; full sentences play through. |
| 2026-06-07 | Cursor Composer — concise + content follow | Voice / 3B | **Concise + content follow** — re-locked short-reply rule in `PERSONA_CORE` + TEACHING MODE v1 (1–2 sentences, max-one-question, no option-dumping/preamble). New `lib/talk/lesson-intent.ts`: `detectLessonContentIntent` + `honorContentIntent` on `/talk` rebuilds plan via `selectLessonTopic(topicHint, excludeTopics)`; rejected topics dropped. `detectExplicitLessonWordRequest` honors "phrase/card to practice". Honesty: forbids fabricated present-context ("were you having basil?"). turn-controller untouched. check:talk 278 + tsc + lint PASS. Pushed `talk/warmth-discipline-reconcile`. | — | Mike: guest /talk — "NOT food, daily phrases" → travel/daily lesson not food; "give me a phrase to practice" → next word+card; replies stay 1–2 sentences. |
| 2026-06-07 | Cursor Composer — warmth + discipline reconcile | Voice / 3B | **Warmth + discipline reconcile** — persona/contracts: SYSTEM OWNS WORD + CARD (Miomi narrates warmly only; warmth = tone). `detectExplicitLessonWordRequest` + `honorExplicitLessonRequest` on `/talk`: "new word" / "show card" → system serves next plan word + card immediately + hidden nudge (no review deflection). `recommendWordPick(forceNewWord)` overrides review-leaning. Transcript sanitizer strips Devanagari/CJK/Cyrillic/Arabic (Thai+Latin kept). turn-controller untouched. check:talk 263 + tsc + lint PASS. Pushed `talk/warmth-discipline-reconcile`. | — | Mike: guest /talk — say "new word" → next plan word + card (not review); English speech bubble has no Devanagari gibberish; Miomi never names off-plan words. |
| 2026-06-07 | Cursor Composer — MEMORY/BOND v1 member | Voice / 3B | **MEMORY/BOND v1 (member, read-only)** — `lib/live/member-context.ts` + `lib/vocab/user-state-read.ts`: bundle at connect via `/api/live-token` (`display_name`, ui/target, CEFR+tier level, introduced/mastered counts, ~3 recent words, `last_seen_at` gap, `isReturning`). Guests get `memberContext: null`. `resolveKickoffAudience` uses real history (not session canvas word count). `buildSystemInstruction` + `buildKickoffPrompt` inject bundle facts + honesty lock (never invent name/history). turn-controller / lesson-plan untouched. check:talk 251 + tsc + lint PASS. | — | Mike: returning member hears name + gap-aware welcome; brand-new member first-time hello; guest unchanged fresh each open. |
| 2026-06-07 | Cursor Composer — Stage A guest language pick | Voice / 3B | **Stage A — wired language choice** — guest pre-lesson pick (Thai/English) with locale pre-highlight + `guest-practice-target` cookie; `resolveSessionLanguages` derives ui/target from pick (fallback en/th only when no pick); `TalkConfig.teach.learning` reaches session + `/api/teach-word` + lesson-plan direction via `teachLearningTarget`; guest UI anchor locked to chosen native. turn-controller untouched. check:talk 234 + tsc + lint PASS. | — | Mike: guest pick English → Thai UI + en_to_th cards; pick Thai → English UI + th_to_en; returning guest skips pick; AdjustSheet "I'm learning" changes target mid-session for members. |
| 2026-06-07 | Cursor Composer — talk refinements round 2 | Voice / 3B | **Three isolated refinements** — (1) `claimLessonWordCard` shared set in `lesson-layer.ts`: tool + backstop + repeat tool calls → exactly one card per normalized word. (2) User bubble commits on `inputTranscription.finished` (fallback: first model chunk); removed `turn_complete` commit; `(turnSeq, roleOrder)` sort kept. (3) Plan order easiest-first: `difficulty_score` asc, `frequency_score` desc tie-break, `prerequisite_words` respected. turn-controller untouched. check:talk 222 + tsc + lint PASS. | — | Mike: guest /talk — one card per word (no dup on backstop/repeat tool); your words appear before her reply; lesson words progress easy → phrase. |
| 2026-06-06 | Cursor Composer — Stage 2 lesson + transcript | Voice / 3B | **Two commits** — (A) `4da0684` Stage 2 themed lesson: `selectLessonTopic` + single-topic `buildLessonPlan`, `nextPlannedWord` never-null until `lesson_complete`, word-lock nudge/contract, `lesson-layer` card backstop (off-tool + suspend-safe cards), `lesson_topic` on teach-word. (B) `f0e9c25` transcript: `turnSeq`+`roleOrder` sort (user above Miomi per turn), finals-only user bubble on `turn_complete`, CJK sanitization kept. turn/handoff reducer untouched. check:talk 214 + tsc + lint PASS. | — | Mike: guest /talk — one food/travel-themed lesson, every taught word gets exactly one card; user bubble stable + above her reply when chunks arrive out of order. |
| 2026-06-06 | Cursor Composer — word card glyph placeholder | Voice | **Word cards** — removed bank `emoji` as card visual; `WordCard` + `WordCardV3` show soft glyph tile (target script) until real-image system. `194312a` pushed main. tsc + lint + check:talk PASS. | — | Mike: /talk + Growth — cards show glyph tile, never emoji; diagnose word-2 card + transcript order/content (read-only trace delivered). |
| 2026-06-06 | Cursor Composer — session continuity + kickoff copy | Voice | **Transport resume** — `session-continuity.ts`: close classification, epoch guard, max 2 reconnects; `MiomiLiveClient` intentionalClose + resume connect (preserves lessonPlan/introducedIdx); `/talk` transparent reconnect + `buildResumePrompt`, mic restore, tap-to-continue; stale onClose ignored. **First-time kickoff** — `buildKickoffPrompt(first_time|returning)`; teaching contract forbids recall framing on first word. check:talk 182 + tsc + lint PASS. | — | Mike: drop Wi‑Fi mid word-2 — cards stay, resume not re-greet; guest first open — no "missed you". |
| 2026-06-06 | Cursor Composer — Stage 1 lesson plan | Voice / 3B | **Stage 1 — deterministic ordered lesson serving** — new `lib/talk/lesson-plan.ts`: `buildLessonPlan` (cardable-only, frequency_score desc + created_at, tier planSize guest=3/free=4/pro=6/pro_max=6), cursor serve via `introduced_idx`, `lesson_complete` when exhausted; `/api/teach-word` + `/api/review-word` plan-based (review rotates introduced plan words + session exclude); `MiomiLiveClient` carries `lessonPlan` + `introducedIdx`. turn-controller / prompts untouched. check:talk 162 + tsc + lint PASS. | — | Mike: guest /talk — up to 3 ordered NEW words then warm wrap-up; review cycles plan words already taught. Stage 2: topic filter + persona nudges. |
| 2026-06-06 | Cursor Composer — teach slug gate + review rotation | Voice / 3B | **Two isolated commits** — (A) `60ef31d`: rewrite `isVocabularySlug` (digit suffix + topic suffix, no `_` gate); `pickWordToIntroduce`/`pickIntroduceCandidate` cardable-only pick + `countUncardableBankRows` log. (B) `7f92d83`: `selectDueReviewCandidate` null `next_spiral_at` → Infinity; session `exclude[]` via `/api/review-word` + `MiomiLiveClient.sessionReviewServed`. turn-controller untouched. check:talk 134 + tsc + lint PASS. | Slugs like `general2` leaked to cards; unscheduled words dominated review; same review word looped. | Mike: /talk — NEW word always cards real gloss; review rotates across due words. |
| 2026-06-06 | Cursor Composer — per-turn latency anchor | Voice / Observability | **Per-turn latency instrumentation** — `user_turn_start` re-anchors each turn (clears stale `model_audio_first`); first `model_audio` after anchor only; `deltaMs = first_audio − user_turn_start` (positive); overlay headline reads latest turn's delta. Teaching/turn logic untouched. check:talk 116 + tsc + lint PASS. | Kickoff was pinning `model_audio_first` once → bogus 0ms/negative overlay. | Mike: one voice turn — overlay shows e.g. `user_turn → first_audio: 1850ms`. |
| 2026-06-06 | Cursor Composer — turn controller deploy | Voice / Observability | **Turn controller shipped** — `lib/live/turn-controller.ts` + `turn-runtime.ts` committed; `/talk` wired via `TurnRuntime`/`dispatchTurn` (replaces racy ref orchestration). SW clone + debug overlay retained. Teaching slug/review untouched. Vercel `4b74fec` Ready — `buildId=4b74fecee23b` on miomika.com. check:talk 109 + tsc + lint PASS. | 366ab8b had reverted to old orchestration (deployment gap, not code bug). | Mike: one voice turn on /talk — overlay shows real `user_turn → first_audio` ms. |
| 2026-06-06 | Cursor Composer — SW clone + debug overlay | Infra / Observability | **Baseline fixes (no teaching logic)** — (1) `sw.js` + generator: clone `Response` synchronously before any async `caches.open`/`put` (fixes stale-while-revalidate "body already used" spam on `/talk`). (2) Debug overlay: full header-bar triple-tap via `onPointerDown` (900ms window, 44px hit zone) + `?` toggle; overlay shows `user_turn_start → model_audio_first` latency line. LOCKED contracts untouched. check:talk 109 assertions + tsc PASS. | — | Mike: /talk — zero SW clone errors in console; triple-tap header or `?` opens overlay; read latency after one voice turn. |
