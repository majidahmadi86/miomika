...
| 2026-07-17 | Cursor — soft daily limit + calm voice | Talk / Voice | **Pushed** — (1) `/talk` daily limit → soft tappable status line (no auto-opening paywall); (2) `prompt.ts` CALM VOICE (≤1 `!`, rare interjections) + FINAL CHECK brevity. `npm run predeploy` PASS. | — | Mike: hit free daily chat cap → teal tappable line opens paywall on tap; listen for calmer TTS punctuation. |
