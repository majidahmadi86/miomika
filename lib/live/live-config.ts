...
export function buildSessionSystemInstruction(
  ...
  const languageMix =
    level === "A1" || level === "A2"
      ? `LEAD IN ${uiName.toUpperCase()}: the learner is a beginner — run the whole session in ${uiName} (explanations, instructions, questions, encouragement all in ${uiName}); ${targetName} appears ONLY as the phrases and lines you are teaching or they are practicing, each taught before it is asked for. NEVER address them in flowing ${targetName} — they cannot follow it yet.`
      : level === "B1"
