...
export type OrchestratorCallbacks = {
  onMicStart: () => void;
  onMicStop: () => void;
  /** Imperative VAD update — must NOT trigger React setState. */
...
export class ConversationOrchestrator {
  ...
  private langListeners = new Set<(lang: TtsLang) => void>();
  private destroyed = false;

  private conversationLang: TtsLang;
  private currentVadConfig: VadConfig = VAD_NORMAL;
  ...
  onConversationLangChange(cb: (lang: TtsLang) => void): () => void {
  ...
  updateOptions(partial: Partial<ConversationOrchestratorOptions>): void {
    const next = { ...this.opts, ...partial };
    const prev = this.opts;
    const unchanged =
      next.isGuest === prev.isGuest &&
      next.authReady === prev.authReady &&
      next.uiLang === prev.uiLang &&
      next.isThaiLeadUser === prev.isThaiLeadUser &&
      next.ttsOn === prev.ttsOn &&
      next.respLength === prev.respLength &&
      next.guestExchanges === prev.guestExchanges &&
      next.guestLimit === prev.guestLimit &&
  ...
  private setConversationLang(lang: TtsLang): void {
    if (this.conversationLang === lang) return;
    this.conversationLang = lang;
    for (const cb of this.langListeners) cb(lang);
  }

  private applyVadConfig(config: VadConfig): void {
    if (
      this.currentVadConfig.positiveSpeechThreshold === config.positiveSpeechThreshold &&
      this.currentVadConfig.negativeSpeechThreshold === config.negativeSpeechThreshold &&
      this.currentVadConfig.redemptionMs === config.redemptionMs
    ) {
      return;
    }
    this.currentVadConfig = config;
  ...
  private playOpening(): void {
    ...
    this.setConversationLang(openerLang);
    ...
    window.setTimeout(() => {
      ...
      void speak(speakText, openerLang, {
        onEnd: () => {
          if (!this.destroyed && this.state === "OPENING") {
            this.transition("IDLE");
            this.resumeListening();
          }
        },
  ...
  private async transcribeAudio(audio: Float32Array): Promise<void> {
    ...
    try {
      ...
      if (!this.destroyed) this.enterErrorState();
    }
  }
  ...
  private async handleEngineResponse(data: MiomiApiResponse): Promise<void> {
    ...
    this.setConversationLang(replyLang);
    ...
    try {
      await Promise.race([
        speak(speakText, replyLang, {
          onEnd: () => {
            if (!this.destroyed && this.state === "AI_SPEAKING") {
              this.transition("IDLE");
              this.resumeListening();
            }
          },
      ...
      if (!this.destroyed && this.state === "AI_SPEAKING") this.enterErrorState();
    } finally {
