...
function TermsEN() {
  return (
    <article>
      <DocTitle title="Terms of Service" date="Last updated: 28 June 2026" />
      ...
      <ul>
        <LI>{`Subscriptions. Pro and Pro Max are subscriptions, billed monthly or yearly. Prices are shown in Thai baht (฿) at checkout, and the price you see at purchase is the price that applies.`}</LI>
        <LI>{`Hour packs. Prepaid voice-time packs are available as one-time purchases.`}</LI>
        <LI>{`Payment. Payments are processed by our payment provider (Stripe). We do not store your full card details.`}</LI>
        <LI>{`Auto-renewal. Subscriptions renew automatically at the end of each billing period (monthly or yearly) until you cancel. We charge the payment method on file at each renewal.`}</LI>
        ...
        <LI>{`Refunds. Except where required by Thai law, fees are non-refundable. Because Miomika has a free guest mode and a free plan so you can try it fully before paying, we do not refund fees or usage already consumed. We will, of course, refund genuine billing errors — duplicate charges, unauthorized charges, or amounts charged in error. If a room session fails to start due to a technical error on our side, the room credit is automatically returned to your account. To stop future renewals, see Cancellation above.`}</LI>
      </ul>
...
function TermsTH() {
  return (
    <article>
      <DocTitle title="ข้อกำหนดในการให้บริการ" date="ปรับปรุงล่าสุด: 28 มิถุนายน 2569" />
      ...
      <H2>5. แพ็กเกจ การชำระเงิน และการต่ออายุ</H2>
      <ul>
        <LI>{`การสมัครสมาชิก: Pro และ Pro Max เป็นการสมัครสมาชิกแบบรายเดือนหรือรายปี ราคาจะแสดงเป็นเงินบาท (฿) ณ ขั้นตอนชำระเงิน และราคาที่คุณเห็นตอนซื้อคือราคาที่ใช้บังคับ`}</LI>
        <LI>{`แพ็กชั่วโมง: แพ็กเวลาสนทนาด้วยเสียงแบบจ่ายล่วงหน้ามีจำหน่ายเป็นการซื้อแบบครั้งเดียว`}</LI>
        <LI>{`การชำระเงิน: ดำเนินการโดยผู้ให้บริการชำระเงินของเรา (Stripe) เราไม่จัดเก็บข้อมูลบัตรของคุณแบบเต็ม`}</LI>
        <LI>{`การต่ออายุอัตโนมัติ: การสมัครสมาชิกจะต่ออายุโดยอัตโนมัติเมื่อสิ้นสุดแต่ละรอบการเรียกเก็บเงิน (รายเดือนหรือรายปี) จนกว่าคุณจะยกเลิก เราจะเรียกเก็บเงินจากวิธีการชำระเงินที่บันทึกไว้ในแต่ละรอบการต่ออายุ`}</LI>
        ...
        <LI>{`การคืนเงิน: ยกเว้นกรณีที่กฎหมายไทยกำหนด ค่าบริการไม่สามารถขอคืนได้ เนื่องจาก Miomika มีโหมดผู้เยี่ยมชมแบบฟรีและแพ็กเกจฟรี ให้คุณได้ทดลองใช้อย่างเต็มที่ก่อนชำระเงิน เราจึงไม่คืนเงินค่าบริการหรือปริมาณการใช้งานที่ใช้ไปแล้ว ทั้งนี้ เราจะคืนเงินให้ในกรณีที่เกิดข้อผิดพลาดในการเรียกเก็บเงินจริง เช่น การเรียกเก็บซ้ำ การเรียกเก็บที่ไม่ได้รับอนุญาต หรือจำนวนเงินที่เรียกเก็บผิดพลาด หากเซสชันห้องไม่สามารถเริ่มได้เนื่องจากข้อผิดพลาดทางเทคนิคจากฝั่งเรา เครดิตห้องจะถูกคืนเข้าบัญชีของคุณโดยอัตโนมัติ หากต้องการหยุดการต่ออายุในอนาคต โปรดดูหัวข้อการยกเลิกด้านบน`}</LI>
      </ul>
