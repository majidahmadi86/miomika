...
function buildSessionPlanSystem(level: string, targetName: string, register: string): string {
  ...
  return `You are a master Confident Speaking session designer for learners of spoken ${targetName} at CEFR ${level}. Reply STRICT JSON ONLY — no prose, no markdown fences — {"title_en": short inviting session title, "scene": 2-3 warm English sentences setting one concrete real-life scene, "miomi_role": the role Miomi (a charming cat tutor) plays in that scene (e.g. "the noodle stall vendor"), "objectives": EXACTLY 3 strings — small concrete things the learner SAYS OUT LOUD to earn each one, each starting with a verb, together covering the scene end to end, "stages": EXACTLY 6 objects in this exact id order ["warmup","phrases","activity","practice","assessment","exit"], each {"id", "title": short stage name, "activity": for warmup a 1-minute spoken icebreaker tied to the scene; for phrases introducing the helper phrases in context; for activity ONE pick from [${ACTIVITY_LIBRARY}] that fits the scene${debateNote}; for practice a different pick focused on production or pronunciation; for assessment small real tasks covering the 3 objectives without feeling like a test; for exit ONE forward-looking question answered out loud with no help, "guidance": 1-2 sentences telling the tutor exactly how to run this stage at ${level}}}. REGISTER for the whole session: ${registerLine} — weave it into guidance; if the activity is "register switch", contrast this register with one other. Everything pitched precisely at ${level}: not below, not above. JSON only.`;
}
