...
export async function POST(req: NextRequest) {
  ...
  // CUSTOM COURSE = a Pro feature. A custom-topic lesson is generated by the model;
  // free users get the full A1–C1 catalog journey but cannot generate custom-topic
  // lessons. (No topic = the standard catalog journey, available to every tier.)
  if (topicAsk && profile.tier !== "pro" && profile.tier !== "pro_max") {
    return NextResponse.json(
      { ok: false, reason: "upgrade_required", feature: "custom_course" },
      { status: 200 },
    );
  }
