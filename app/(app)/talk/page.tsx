...
export default function TalkPage() {
  ...
  const [vadConfig, setVadConfig] = useState<VadConfig>(DEFAULT_VAD);
  ...
  useEffect(() => {
    ...
    const orch = new ConversationOrchestrator({
      ...
      callbacks: {
        ...
        onVadConfig: (cfg) => {
          setVadConfig(cfg);
          micRef.current?.setVadConfig(cfg);
        },
        onGuestLimit: () => {
          micRef.current?.stop();
  ...
  useEffect(() => {
    const lang = orchRef.current?.getConversationLang();
    if (lang) setConversationLang(lang);
  }, [orchState]);
